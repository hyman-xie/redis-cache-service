package com.hyman.cache.redis.impl;
		...
		return redisTemplate.execute(new RedisCallback<List<T>>() {
			...
			public List<T> doInRedis(RedisConnection connection) throws DataAccessException {
				...
				if(keySet.size()<=Constants.REDIS_MGET_LIMIT){
					List<byte[]> results= connection.mGet(keySet.toArray(new byte[keySet.size()][]));
					if (results == null) {
						return null;
					}
					...
					for(int i=0;i<limitCount;i++){
						connection.mGet(keySet.subList(i*Constants.REDIS_MGET_LIMIT, (i+1)*Constants.REDIS_MGET_LIMIT).toArray(new byte[Constants.REDIS_MGET_LIMIT][]));
					}
					if(leftCount>0){
						connection.mGet(keySet.subList(limitCount*Constants.REDIS_MGET_LIMIT, keySet.size()).toArray(new byte[leftCount][]));
					}
